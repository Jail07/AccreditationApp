...
class DataProcessor:
    def removeExtraSpaces(self, value):
        """
        Удаляет лишние пробелы, нормализует строку и обрабатывает пробелы вокруг дефисов.
        """
        ...
        # Нормализация строки (убираем диакритические символы)
    ...
    def normalizeDate(self, date_str):
        """Приводит дату к стандартному формату."""
        if pd.isna(date_str) or date_str == '':
            return None
        try:
            clean_date_str = re.sub(r'[^\d.]', '', str(date_str)).strip()  # Удаляем лишние символы
    ...
    def checkDuplicates(self, df, ref_df=None, key_columns=None, log_func=None):
        """Проверяет дублирование в файле и с БД."""
        ...
        # Все остальные корректные данные
    ...
    def detectSequentialDates(self, df, date_column='Дата рождения', max_allowed_sequence=2):
        """
        Находит строки с подозрительными последовательностями дат.
        """
