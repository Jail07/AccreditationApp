...
class AccreditationApp(QWidget):
    ...
    def remove_accents(value):
        """
        Удаляет диакритические знаки (ударения) из строки.
        """
        if not isinstance(value, str):
            return value

        # Приводим строку в нормализованный вид и удаляем диакритики
        normalized = unicodedata.normalize('NFD', value)
        without_accents = ''.join(ch for ch in normalized if not unicodedata.combining(ch))
        return without_accents

    def removeExtraSpaces(self, value):
        """Удаляет лишние пробелы, неразрывные пробелы, диакритики и нормализует строку"""
        value = value.replace('\u00A0', ' ')  # Заменяет неразрывные пробелы на обычные
        value = unicodedata.normalize('NFD', value)  # Нормализует символы с диакритиками
        value = ''.join(ch for ch in value if not unicodedata.combining(ch))  # Убирает диакритики
        value = value.strip()  # Убирает пробелы с начала и конца
        value = re.sub(r'\s*-\s*', '-', value)  # Убирает пробелы вокруг дефиса
